...
def gen_addr(num_chan, num_bank, num_rank, num_col, num_row, num_addr, num_sa):
    ...
    #                 count += 1
    #                 print("count: %d, rank: %s, chan: %s, bank: %s" % (count, chan,rank,bank))
    #                 # append zeros
    ...
    #         chan = "00"
    #         rank = "000"
    #         candidate_banks = ["000","001"]
    #         for b in range(0, len(candidate_banks)):
    ...
    # # baseline DRAM-CAM, each query walks through 6 banks 8 times
    for i in range(num_addr):
        for ch in range(0, len(chan_lst)):
            chan = chan_lst[ch]
            for ra in range(0, len(rank_lst)):
                rank = rank_lst[ra]
                for ba in range(0, len(bank_lst)):
                    bank = bank_lst[ba]
                    row = str(random.choice(row_lst))
                    col = str(random.choice(col_lst))
                    addr_tmp = row+col+rank+bank+chan
